...
public class Bullet {
    ...
    public Bullet(float v,float pixelx,int[] bulletcolor,float radian) {
        this.v = v;
    ...
    public void collide( float[]window, ArrayList<Tank> tanks){
        ...
        System.out.println("window[i]: "+(int)window[(int)pixelx]);
        ...
        //判定爆炸
        if(Math.abs((int)pixely)>=(int)window[(int)pixelx]){
            ...
            for(int j= 0; j< 864 ; j++){
                ...
                if(dist <=30){
                    ...
                    System.out.println("r for bullet: " + r);
